...
├── RELEASE_NOTES.md        # v0.2.0 发布说明（总结/明细/遗留问题/改进方案）
├── requirements.txt        # 依赖（transformers>=5.3.0、torch>=2.5.0 是硬约束！）
├── .gitignore              # 忽略 缓存/ 结果/ __pycache__/
│
├── docs/
│   └── 实验报告_v0.2.0.md # 完整实验报告（v0.2.0）
├── 配置/
│   └── 配置.py             # ★参数总控：模型ID、数据规模、补丁清单、开关（v0.2.0只保留策略A）
├── 工具/
...
├─ 步骤 7: 生成补丁（策略A:2/4/6%）——只存被选位置的 Δ=B−A
...
- 策略B：Fisher 重要度**全局 Bottom K**（v0.2.0 起从默认补丁清单移除，已验证补丁为空≈基座）
- 策略C：**每张量内部**取 高2% + 低2% + 中2%（v0.2.0 起从默认补丁清单移除，恢复率88.3%低于策略A）
...
16. **v0.2.0 起只保留策略A**：策略B（低重要度）已验证补丁为空（Δ全部低于阈值）、准确率≈基座；策略C（混合）恢复率88.3%低于策略A的96.8%。两者从默认补丁清单移除，代码仍保留（可手动加回配置），未来聚焦策略A优化。
...
| 白皮书论断 | 我们怎么验证 | 成功标准 |
| --- | --- | --- |
| Top 2~6% 参数替换逼近全量微调 | 策略A 补丁恢复率 | 恢复率≥90%（本任务上）✅已验证 |
| 策略B 只改风格不改能力 | 策略B 补丁准确率 | 准确率≈基座（差距<5%）✅已验证 |
| 无低秩瓶颈，优于 LoRA | 补丁 vs LoRA 准确率+体积 | 同体积下补丁更准 ⚠️LoRA失败待修 |
| 毫秒级切换 | 显存内覆盖计时 | 报告实测毫秒数 ✅已验证(17ms) |
| 补丁体积小 | 文件大小统计 | 与 LoRA 适配器对比 ✅已验证(0.6MB) |
...
4. **LoRA 的 target_modules 是运行时探测的**（q_proj 等），若 Qwen3.5 命名不同会报错，用户反馈后调整。**v0.2.0 发现 LoRA 完全失败（准确率≈基座），疑似 target_modules 探测到错误层，列为高优先级遗留问题。**
5. **策略C 的"中段"是简化实现**（每张量局部而非全局），量级正确但口径与 A/B 不同。v0.2.0 起从默认清单移除。
...
- **[2026-09-03] v0.1.10 修复：策略C选位阶段卡住（argsort全排序太慢）+ 增加选位进度打印**
  - **用户现象**：步骤 7 生成补丁时，策略C（高2%+低2%+中2%）选位阶段卡很久，看不到 CPU/GPU 在跑。
  - **根因**：`补丁/生成补丁.py` 的 `_混合选三段` 中，中分段用了 `torch.argsort(平展)`（全排序，O(n log n)）。对词嵌入层这种 5 亿参数的大张量，在 CPU 上做全排序要跑几分钟，且没有任何进度打印，用户以为卡死了。对比策略A/B用的是 `torch.topk`（只取前K，比全排序快得多）。
  - **修复**：
    - `补丁/生成补丁.py`：策略C的中分段从 `torch.argsort` 全排序改为**随机采样近似**（`torch.randint` + `torch.unique`）。中段=既不高也不低的参数，随机取效果等价（高/低各占2%，随机取2%与高/低重叠的概率<0.1%，可忽略），且毫秒级完成。
    - `补丁/生成补丁.py`：`_全局选Top`（策略A/B）和 `_混合选三段`（策略C）都增加进度打印，每处理 5 个参数张量打印一次（含当前张量名和参数数量），让用户看到在跑。
    - `补丁/生成补丁.py`：`生成补丁文件` 在选位置前打印"正在选择参数位置..."。
  - **用户操作**：`git pull` 后重跑 `python 运行/主入口.py`。这次生成补丁时会看到"正在选择参数位置..."+"选位进度：1/XX 个参数张量（当前：xxx，xxx 个参数）"，策略C不会再卡几分钟。

- **[2026-09-03] v0.2.0 正式发布：理论成立，测试通过，砍掉B/C策略**
  - **实验结果（用户侧跑完v0.1.10）**：
    - 基座A 37.5%，全量微调B 100%
    - 策略A_2% 97.7%（恢复率96.3%），策略A_4% 98.7%（恢复率97.9%，最佳），策略A_6% 98.0%（恢复率96.8%）
    - 策略B_2% 38.8%（恢复率2.1%，补丁为空0MB），策略C_6% 92.7%（恢复率88.3%）
    - LoRA r=16/r=64 均37%左右（完全失败，疑似target_modules探测问题）
    - 补丁文件仅0.59-0.66MB（阈值过滤后实际替换<0.01%参数），切换耗时16-18ms
  - **核心结论**：钻石架构核心机制（Fisher选位+Δ替换）验证通过，白皮书两个核心论断均得到实验支持。
  - **v0.2.0变更**：
    - `配置/配置.py`：补丁清单只保留策略A（2%/4%/6%），移除策略B和策略C（B已验证补丁为空≈基座，C恢复率88.3%低于A的96.8%）
    - 新增 `docs/实验报告_v0.2.0.md`：完整实验报告
    - 新增 `RELEASE_NOTES.md`：发布说明（总结/明细/遗留问题/改进方案）
    - `MEMORY.md`：更新v0.2.0记录
  - **遗留问题**：①LoRA对照组失败（target_modules探测，高优先级）；②实际替换占比显示0.00%（显示精度，中优先级）；③任务过于简单（合成三分类，低优先级）；④阈值过滤过于激进（低优先级）
  - **未来路线图**：短期修LoRA+显示+阈值可调+稀疏再训练；中期接入真实数据集（MetaMath/MBPP）+多补丁并发+补丁压缩；长期完整多专家系统+Fisher预计算+补丁分发